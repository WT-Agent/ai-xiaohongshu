  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">赛道领域</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">笔记类型</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">小红书爆款图文与算法流量通关指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 小红书全能文案与视觉专家');
...
const metricsList = [
  {
    "key": "viralHeadlineAttractiveness",
    "label": "爆款标题吸引力"
  },
  {
    "key": "visualCoverLayoutDesign",
    "label": "封面视觉美学度"
  },
  {
    "key": "seedingConversionEfficiency",
    "label": "种草转化效率"
  },
  {
    "key": "seoHashtagRelevance",
    "label": "SEO热搜词契合度"
  },
  {
    "key": "communityEngagementScore",
    "label": "社群互动控评分"
  }
];
const guideRules = [
  {
    "title": "封面大字报是第一生产力",
    "advice": "首图采用高对比度背景配上粗体大字标题（如“听我一句劝！”），突出核心吸引点",
    "avoid": "切忌封面图没有文字或画面混乱杂乱无章"
  },
  {
    "title": "文案排版网感十足",
    "advice": "多用短句，加入分界线与醒目标签，保持闺蜜间倾诉分享的真诚口吻",
    "avoid": "避免像论文或新闻稿一样死板冷硬"
  },
  {
    "title": "SEO 关键词自然嵌入",
    "advice": "在标题、正文前 50 字及结尾标签中嵌入核心搜索词（如“减脂早餐”），抢占搜索流量",
    "avoid": "严禁乱堆无关热门标签引发平台判定违规"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[XIAOHONGSHU_SCORES\][\s\S]*?\[\/XIAOHONGSHU_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[XIAOHONGSHU_SCORES\](.*?)\[\/XIAOHONGSHU_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('xiaohongshu_history_records');
...
const saveHistory = () => {
  localStorage.setItem('xiaohongshu_history_records', JSON.stringify(historyList.value));
};
