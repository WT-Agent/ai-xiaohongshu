  ...
  <div class="app-container">
          ...
          <div v-for="item in historyList" :key="item.id" class="history-card">
            ...
            <div class="h-card-body">
              <div class="h-card-nomad-title">
                <span class="h-city-tag">主题: {{ item.destination }}</span>
                <span class="h-score-badge">爆款评级: {{ getAverageScore(item) }}</span>
              </div>

              <p class="h-card-excerpt"><strong>爆款摘要：</strong>{{ cleanExcerpt(item.output) }}</p>
            </div>
      ...
      <div v-else>
        ...
        <div v-if="!result" class="divination-setup">
          <div class="selector-group">
            <label class="selector-label">种草产品或主题 (如：抗初老玻色因精华液、深色玻璃拟态 UI 设计干货、成都3天2晚隐秘游避坑攻略)</label>
            <input 
              type="text" 
              v-model="destination" 
              class="city-text-input" 
              placeholder="例如：抗初老玻色因精华液，主打提亮修护"
            />
          </div>

          <div class="selector-group">
            <label class="selector-label">受众群体与文案细节要求 (如：针对20-30岁精致女生，标题要用感叹号与吸睛冲突词，正文排版有呼吸感，带有爆款话题标签)</label>
            <textarea 
              v-model="userInput" 
              placeholder="请输入您的受众群体或排版要求。例如：针对20-30岁精致女生，标题要用感叹号与吸睛冲突词，正文排版有呼吸感，带有爆款话题标签。"
              rows="5"
            ></textarea>
          </div>

          <div class="selector-group">
            <label class="selector-label">选择小红书爆款文案流派</label>
          ...
          >
            {{ loading ? '爆款操盘推演中...' : '提交主题并生成小红书爆款文案' }}
          </button>
        ...
        <div v-else class="divination-result">
          ...
          <div class="stamp-section">
            <div class="stamp-canvas">
              ...
              >
                ...
                <path d="M 80 115 C 60 95 40 75 40 60 C 40 45 55 35 70 45 C 80 52 80 52 80 52 C 80 52 80 52 90 45 C 105 35 120 45 120 60 C 120 75 100 95 80 115 Z" fill="#ef4444" />
                <text x="80" y="130" font-size="12" font-weight="bold" fill="#ef4444" text-anchor="middle">爆款红心打卡</text>
              </svg>
              <!-- 漂浮在看动效 (严格无 Emoji) -->
            ...
            <div class="merit-counter-display">
              累计小红书爆款红心：<strong style="color: #ef4444;">{{ totalCheckin }}</strong> 次
              <p class="wood-fish-tip">点击上方爆款红心，点赞收藏为爆款图文打卡</p>
            </div>
          </div>

          <!-- 单轨爆款指标多维评估看板 (AI共识判定) -->
          <div v-if="aiScores" class="comparison-dashboard">
            <h3 class="dashboard-title">小红书爆款图文多维评估 (AI共识判定)</h3>
          ...
          <div class="result-header">
            <div class="result-title-group">
              <span class="result-title">小红书爆款图文与种草</span>
    ...
    <div v-if="showPrivacy" class="modal-overlay" @click.self="showPrivacy = false">
      <div class="modal-content">
        ...
        <div class="modal-text-content modal-scroll-area">
          <p>我们非常重视您的隐私。您在本应用中输入的种草产品、受众细节等提示词仅用于实时大模型分析与爆款文案生成，我们不会在服务器端进行永久存储或记录。</p>
          <p>为了记录您的爆款历史、免费额度和打卡数据，本应用会在您的浏览器本地（localStorage）保存相关状态。</p>
        </div>
    ...
    <div v-if="showTerms" class="modal-overlay" @click.self="showTerms = false">
      <div class="modal-content">
        ...
        <div class="modal-text-content modal-scroll-area">
          <p>欢迎使用我们的 AI 小红书爆款图文与种草神器辅助创作微应用。生成的文案标题与话题标签仅供运营创作与种草启发参考，切勿发布违规虚假广告。</p>
        </div>
    ...
    <div v-if="showShareGuide" class="share-guide-overlay" @click="handleShareClose">
      ...
      <div class="share-guide-content">
        ...
        <p class="share-guide-sub">分享这款好用的 AI 小红书爆款图文与种草微应用</p>
      </div>
    </div>
  </div>
</template>
...
const destination = ref('抗初老玻色因精华液，主打提亮修护防暗沉');
const userInput = ref('');
...
const totalCheckin = ref(parseInt(localStorage.getItem('xhs_total_heart_stamp') || '0', 10));
const floatingItems = ref<{ id: number; text: string; x: number; y: number }[]>([]);
let floatingId = 0;

// 爆款指标评估列表
const metricsList = [
  { key: 'clickRate', label: '爆款标题点击率 (Click Rate)' },
  { key: 'seedingValue', label: '种草转化能力 (Seeding Value)' },
  { key: 'readability', label: '排版阅读体验 (Readability)' },
  { key: 'tagRelevance', label: '话题标签契合度 (Tag)' },
...
const styleOptions = [
  ...
  { label: '干货硬核整理 (条理清晰/步骤明确/清单式整理/收藏率极高)', value: '干货硬核整理' },
  { label: '情绪共鸣避坑 (真实倾诉语气/指出避坑误区/拉近心理距离)', value: '情绪共鸣避坑' },
  { label: '沉浸感日常 Vlog (第一人称生活化沉浸语气/充满温馨氛围感)', value: '沉浸感日常 Vlog' },
  { label: '极简图文金句 (审美密度极高/标题抓人/视觉卡片感文案)', value: '极简图文金句' }
];
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('xhs_history_records');
...
const saveHistory = () => {
  localStorage.setItem('xhs_history_records', JSON.stringify(historyList.value));
};

const addHistoryRecord = () => {
  const matched = styleOptions.find(o => o.value === activeStyle.value);
  const styleLabel = matched ? matched.label : '爆款流派';
...
const clearAllHistory = () => {
  if (confirm('确认清空所有历史小红书爆款报告吗？此操作不可恢复。')) {
    historyList.value = [];
    saveHistory();
  }
};
...
const getAverageScore = (item: HistoryItem) => {
  ...
  const avg = (s.clickRate + s.seedingValue + s.readability + s.tagRelevance + s.interactivity) / 5;
  return avg.toFixed(1);
};

// 纯前端利用 Web Audio API 动态合成清脆点赞啵啵声效 (正弦波 800Hz 快速下降至 400Hz)
const stampCheckin = () => {
  ...
  // 增加爆款红心打卡计数
  totalCheckin.value++;
  localStorage.setItem('xhs_total_heart_stamp', totalCheckin.value.toString());

  // 漂浮动效 (严格无 Emoji)
  const id = floatingId++;
  const x = (Math.random() - 0.5) * 60;
  const y = -40 - Math.random() * 20;
  floatingItems.value.push({ id, text: '爆款红心点赞收藏', x, y });
  setTimeout(() => {
...
const parseAIScores = (text: string) => {
  const match = text.match(/\[XIAOHONGSHU_SCORES\](.*?)\[\/XIAOHONGSHU_SCORES\]/);
  if (match) {
    ...
    return {
      clickRate: scores.clickRate || 3,
      seedingValue: scores.seedingValue || 3,
      readability: scores.readability || 3,
      tagRelevance: scores.tagRelevance || 3,
      interactivity: scores.interactivity || 3
    };
  }
  return null;
};
...
const cleanResponseText = (text: string) => {
  return text.replace(/\[XIAOHONGSHU_SCORES\][\s\S]*?\[\/XIAOHONGSHU_SCORES\]/gi, '').trim();
};
...
const handleGenerate = async () => {
  ...
  try {
    const fullPrompt = `【种草产品或主题】：${destination.value}\n【受众群体与细节要求】：${userInput.value}\n【选定爆款文案流派】：${activeStyle.value}\n${promptTopic.value}`;
